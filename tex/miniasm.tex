...
Given two sequences $s$ and $s'$ that share a long common substring, let
$\{(i,i')\}$ be the set of positions of matching minimizers. It 
...
%			Append $(x,t,i',i)$ to $\mathcal{A}$\;
	...
	\For (\Comment*[f]{Cluster minimizer matches}) {$e=1$ \KwTo $|\mathcal{A}|$} {
		\If{$e=|\mathcal{A}|$ {\bf or} $\mathcal{A}[e+1].t\not=\mathcal{A}[e].t$ {\bf or} $\mathcal{A}[e+1].r\not=\mathcal{A}[e].r$ {\bf or} $\mathcal{A}[e+1].c-\mathcal{A}[e].c>g$} {
			$\mathcal{C}\gets$the maximal colinear subset of $\mathcal{A}[b..e]$\;
...
Algorithm~4 describes how to map a query against the hash table of target
minimizers.
