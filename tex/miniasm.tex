...
\subsection{Assembling bacterial data}
...
With four CPU cores, it took miniasm 17 seconds to assemble the 30-fold
PBcR-PB-ec data set and 3 minutes to assemble the 160-fold PB-ecoli data set.
PBcR is 500 times slower on PBcR-PB-ec and 35 times slower on PB-ecoli.
The performance is worse on low-coverage input because PBcR automatically
switches to the slower sensitive mode. Here we should remind readers that
without an error correction stage, the contig sequences generated by miniasm
are of much lower accuracy in comparison to PBcR. The speed comparison is not
fair. Nonetheless, miniasm is still tens of times faster than PBcR excluding
