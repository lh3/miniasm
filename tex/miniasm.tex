...
This experiment evaluates both the sensitivity and the specificity of minimap:
if minimap had low sensitivity, it would miss the BWA-MEM mapping completely;
if minimap had low specificity, its best mapping would often be a wrong
mapping.
...
For this data set, minimap takes 27GB RAM at the peak. As minimap loads at most
4Gbp bases to index, it will not use more RAM given more data set. The memory
used by miniasm is proportional to the number of overlaps.  Although it only
takes 1.3GB RAM here, it will become the limiting factor for larger data sets.
